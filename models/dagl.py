...
class RR(nn.Module):
    def __init__(self, conv=common.default_conv):
        ...
        m_body = [
            common.ResBlock(
                conv, n_feats, kernel_size, nn.PReLU(), res_scale=1
            ) for _ in range(n_resblocks // 2)
        ]
        m_body.append(msa)
        for i in range(n_resblocks // 2):
            m_body.append(common.ResBlock(conv, n_feats, kernel_size, nn.PReLU(), res_scale=1))
...
class CES(nn.Module):
    def __init__(self,in_channels,num=4):
        ...
        # self.input = nn.Conv2d(3, 64, 3, 1, 1)
